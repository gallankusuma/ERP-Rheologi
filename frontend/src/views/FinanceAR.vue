    ...
    <main class="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
      <div class="flex justify-between items-center mb-6">
        <div>
          ...
          <p class="text-sm text-gray-500 mt-1">Outstanding receivables from invoices and payment tracking</p>
        </div>
        <button @click="showAdd = true" class="px-4 py-2 bg-blue-600 text-white rounded-md text-sm hover:bg-blue-700">+ Create AR</button>
      </div>

      <!-- Summary -->
      <div class="grid grid-cols-1 sm:grid-cols-4 gap-4 mb-6">
        <div class="bg-white shadow rounded-lg p-4">
          <p class="text-xs text-gray-500 uppercase">Total Receivable</p>
          <p class="text-2xl font-bold">{{ fmt(totals.total) }}</p>
        </div>
        <div class="bg-white shadow rounded-lg p-4">
          <p class="text-xs text-green-600 uppercase">Received</p>
        ...
        <table class="min-w-full divide-y divide-gray-200">
          ...
          <tbody class="divide-y divide-gray-200">
            <tr v-for="a in filtered" :key="a.id" class="hover:bg-gray-50" :class="isOverdue(a) ? 'bg-red-50' : ''">
              <td class="px-4 py-3 text-sm font-medium text-gray-900">{{ a.invoice_number }}</td>
              <td class="px-4 py-3 text-sm text-gray-600">{{ a.customer_name }}</td>
              ...
              <td class="px-4 py-3 text-right">
                <button v-if="a.status !== 'paid'" @click="openPay(a)" class="text-green-600 hover:underline text-xs">Receive</button>
              </td>
            </tr>
            <tr v-if="!filtered.length"><td colspan="8" class="text-center py-8 text-gray-400">No receivables</td></tr>
          </tbody>
        </table>
      </div>
      ...
      <div v-if="payTarget" class="fixed inset-0 bg-gray-600 bg-opacity-50 flex items-center justify-center z-50">
        <div class="bg-white rounded-lg shadow-xl p-6 w-full max-w-sm">
          <h3 class="text-lg font-bold mb-4">Record Payment Received</h3>
          <p class="text-sm text-gray-600 mb-2">{{ payTarget.invoice_number }} — {{ payTarget.customer_name }}</p>
          ...
          <div class="flex justify-end gap-3 pt-4">
            ...
            <button @click="confirmPay" class="px-4 py-2 bg-green-600 text-white rounded-md text-sm hover:bg-green-700">Record</button>
          </div>
        </div>
      </div>
      ...
      <div v-if="showAdd" class="fixed inset-0 bg-gray-600 bg-opacity-50 flex items-center justify-center z-50">
        <div class="bg-white rounded-lg shadow-xl p-6 w-full max-w-md">
          <h3 class="text-lg font-bold mb-4">Create AR Record</h3>
          <form @submit.prevent="createAR" class="space-y-3">
            <div>
              <label class="block text-sm font-medium text-gray-700">Invoice ID</label>
              <input v-model.number="form.invoice_id" type="number" required class="mt-1 w-full px-3 py-2 border rounded-md text-sm" />
            </div>
...
const form = ref({ invoice_id: '', due_date: '', amount: 0 });
...
const overdue = computed(() => store.ar.filter(a => isOverdue(a)));

const totals = computed(() => ({
  total: store.ar.reduce((s, a) => s + Number(a.amount || 0), 0),
  paid: store.ar.reduce((s, a) => s + Number(a.paid_amount || 0), 0),
  outstanding: store.ar.reduce((s, a) => s + Number(a.amount || 0) - Number(a.paid_amount || 0), 0),
}));
...
const filtered = computed(() => {
  if (filter.value === 'all') return store.ar;
  if (filter.value === 'overdue') return overdue.value;
  return store.ar.filter(a => a.status === filter.value);
});

const openPay = (a: any) => { payTarget.value = a; payAmount.value = a.amount - (a.paid_amount || 0); };
const confirmPay = async () => { await store.payAR(payTarget.value.id, payAmount.value); payTarget.value = null; };

onMounted(() => store.fetchAR());

const createAR = async () => { await store.createAR(form.value); showAdd.value = false; };
</script>
