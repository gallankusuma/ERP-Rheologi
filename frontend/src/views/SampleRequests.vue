          ...
          <tbody class="divide-y divide-slate-100">
            <tr v-for="req in requests" :key="req.id" class="hover:bg-slate-50/50 transition-colors cursor-pointer" @click="viewDetails(req)">
              ...
              <td class="p-4 text-right space-x-2" @click.stop>
                ...
                <button v-if="req.status === 'Requested'" @click="deleteRequest(req)" class="text-slate-400 hover:text-red-600 transition-colors" title="Delete">
...
const deleteRequest = async (req: any) => {
  if (req.status !== 'Requested') {
    alert('Only requests with status Requested can be deleted');
    return;
  }
  if (!confirm(`Delete sample request ${req.request_number}? This cannot be undone.`)) return;
  try {
    await api.delete(`/sample-requests/${req.id}`);
    fetchRequests();
  } catch (error: any) {
    alert(error.response?.data?.error || 'Failed to delete sample request');
  }
};
