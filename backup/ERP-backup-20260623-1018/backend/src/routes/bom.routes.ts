...
router.post('/', authMiddleware, async (req: Request, res: Response) => {
  try {
    const { product_id, product_name, notes, details } = req.body;
    ...
    const headerResult = await dbRun(
      'INSERT INTO bom_headers (product_id, product_name, status, notes, created_by) VALUES (?, ?, ?, ?, ?)',
      [product_id || null, product_name, 'ACTIVE', notes || null, createdBy]
    );
...
router.put('/:id', authMiddleware, async (req: Request, res: Response) => {
  try {
    const { product_id, product_name, notes, details, status } = req.body;

    // Update BOM header (always update product_name and product_id if provided)
    await dbRun(
      `UPDATE bom_headers SET 
       ...
       WHERE id = ?`,
      [product_id || null, product_name || null, notes !== undefined ? notes : null, status || null, req.params.id]
    );

    // Update BOM details ONLY if details array is provided and non-empty
    // This prevents accidental deletion when only editing header
    if (Array.isArray(details) && details.length > 0) {
