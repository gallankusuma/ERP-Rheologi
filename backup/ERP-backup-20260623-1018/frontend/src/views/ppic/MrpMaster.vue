...
const saveAll = async () => {
  ...
  try {
    ...
    await api.put('/ppic/mrp', { entries, year: selectedYear.value });
    isDirty.value = false;
    alert('✓ MRP saved!');
  } catch (err: any) { alert(err?.response?.data?.error || 'Failed to save MRP'); }
...
const generatePR = async () => {
  ...
  try {
    ...
    alert(err?.response?.data?.error || 'Failed to generate PR');
  } finally {
