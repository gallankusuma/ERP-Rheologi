...
async function openDetail(id: number) {
  try {
    const res = await fetch(`/api/qc/fpa/${id}`, { headers: getAuthHeaders() });
    if(res.ok) {
      const data = await res.json();
      detailData.value = data.data;
      if(detailData.value.results) {
        detailData.value.results.forEach((r: any) => {
          r.is_pass = r.is_pass === 1 || r.is_pass === true;
        });
      }
