            ...
            <tr v-for="pr in store.purchaseRequests" :key="pr.id" class="hover:bg-gray-50">
              ...
              <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-2">
                ...
                <button
                  ...
                  class="inline-flex items-center px-3 py-1 rounded text-white text-xs"
                  :class="allSpecsApproved(pr) ? 'bg-purple-600 hover:bg-purple-700' : 'bg-gray-400 hover:bg-gray-500'"
                  :disabled="submitting"
                  :title="allSpecsApproved(pr) ? 'Convert to PO' : 'Spec belum approved 2/2'"
                >
                  {{ allSpecsApproved(pr) ? 'Convert to PO ✓' : 'Spec ⚠' }}
                </button>
              ...
              <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                  <tr>
                    ...
                    <th class="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider" style="min-width: 160px;">Attachment</th>
                    <th class="px-4 py-2 text-center text-xs font-medium text-gray-500 uppercase tracking-wider" style="min-width: 180px;">Spec Approval</th>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                  <tr v-for="(item, idx) in formItems" :key="idx">
                    ...
                    <td class="px-4 py-2 align-top pt-2" style="min-width: 160px;">
                      <!-- Has attachment: show filename + view/remove -->
                      <div v-if="item.attachment" class="flex items-center gap-1 flex-wrap">
                        <a :href="apiBaseUrl + item.attachment" target="_blank"
                          class="inline-flex items-center gap-1 px-2 py-1 rounded text-xs font-medium bg-blue-50 text-blue-700 hover:bg-blue-100 ring-1 ring-inset ring-blue-200 max-w-[140px] truncate" :title="item.attachmentName || 'View file'">
                          ...
                          <span class="truncate">{{ item.attachmentName || 'File' }}</span>
                        </a>
                        <button v-if="!isEditing" @click="removeItemAttachment(idx)" class="text-red-400 hover:text-red-600" title="Remove attachment">
                          <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
                        </button>
                      </div>
                      <!-- No attachment: show upload button -->
                      <label v-else-if="!isEditing" class="inline-flex items-center gap-1 px-2 py-1 rounded text-xs font-medium bg-gray-50 text-gray-600 hover:bg-gray-100 ring-1 ring-inset ring-gray-200 cursor-pointer transition">
                        <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12"/></svg>
                        Upload
                        <input type="file" class="hidden" accept=".pdf,.jpg,.jpeg,.png,.doc,.docx,.xls,.xlsx" @change="uploadItemAttachment(idx, $event)" />
                      </label>
                      <span v-else-if="isEditing" class="text-xs text-gray-400">—</span>
                    </td>
                    <td class="px-4 py-2 align-top pt-2 text-center" style="min-width: 180px;">
                      <template v-if="item.attachment">
                        <!-- Approval status badge -->
                        <span class="inline-flex items-center px-2 py-0.5 rounded-full text-[11px] font-semibold mb-1"
                          :class="specApprovalBadgeClass(item.specApproval || 0)">
                          {{ specApprovalLabel(item.specApproval || 0) }}
                        </span>
                        <!-- Approver names -->
                        <div v-if="item.specApprover1" class="text-[10px] text-gray-400 leading-tight">L1: {{ item.specApprover1 }}</div>
                        <div v-if="item.specApprover2" class="text-[10px] text-gray-400 leading-tight">L2: {{ item.specApprover2 }}</div>
                        <!-- Approve / Reject buttons -->
                        <div class="mt-1 flex items-center justify-center gap-1">
                          <button
                            v-if="canApproveSpec(item.specApproval || 0)"
                            @click="approveItemSpec(idx)"
                            class="px-2 py-0.5 text-[10px] font-medium bg-green-600 text-white rounded hover:bg-green-700"
                            :disabled="submitting"
                          >✓ Approve</button>
                          <button
                            v-if="canRejectSpec(item.specApproval || 0)"
                            @click="rejectItemSpec(idx)"
                            class="px-2 py-0.5 text-[10px] font-medium bg-red-500 text-white rounded hover:bg-red-600"
                            :disabled="submitting"
                          >✕ Reject</button>
                        </div>
                      </template>
                  ...
                  <tr v-if="formItems.length === 0">
                    <td colspan="9" class="px-4 py-4 text-center text-gray-500 text-sm">Tambah minimal 1 item.</td>
                  </tr>
                </tbody>
              </table>
            </div>
          ...
          <div class="border border-gray-200 rounded-lg">
            <div class="px-4 py-3 bg-gray-50 border-b flex items-center justify-between">
              ...
              <div v-if="currentPR" class="flex items-center space-x-2">
                ...
                <button @click="showAddBidModal = true; selectedBidItems = formItems.map((_: any, i: number) => i)" class="px-3 py-1 text-sm bg-white border border-gray-300 rounded-lg hover:bg-gray-50">+ Add Vendor</button>
              </div>
            </div>
            <div class="overflow-x-auto">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                  <tr v-for="(item, rowIdx) in bidItemRows" :key="rowIdx" class="hover:bg-gray-50">
                    ...
                    <template v-for="bid in prBids" :key="'c-'+bid.id+'-'+rowIdx">
                      <template v-if="hasBidItem(bid, rowIdx)">
                        <td class="px-2 py-1 border-l-2" :class="isItemWinner(bid.id, rowIdx) ? 'border-green-300 bg-green-50' : 'border-gray-200'">
                          <div class="flex items-center gap-1">
                            <input
                              type="radio"
                              :name="'item-winner-'+rowIdx"
                              :checked="isItemWinner(bid.id, rowIdx)"
                              @change="selectItemWinner(bid, rowIdx)"
                              class="w-3.5 h-3.5 text-green-600 border-gray-300 cursor-pointer shrink-0"
                              title="Select as winner for this item"
                            />
                            <div class="relative flex-1">
                              <input
                                ...
                                :class="[
                                  isItemWinner(bid.id, rowIdx) ? 'bg-green-50 border-green-400' : 'border-gray-300',
                                  getBidItem(bid, rowIdx).notes === 'Auto-filled from Vendor Price List' ? 'border-blue-400' : ''
                                ]"
                                placeholder="0"
                        ...
                        <td class="px-2 py-2 text-right text-sm" :class="isItemWinner(bid.id, rowIdx) ? 'bg-green-50 text-green-700 font-semibold' : 'text-gray-700'">
                          {{ formatCurrency(getBidItem(bid, rowIdx).total_price || 0) }}
                        </td>
                      </template>
                      <template v-else>
                        <td class="px-2 py-1 border-l-2 border-gray-200 bg-gray-50">
                          <span class="text-xs text-gray-300 italic">—</span>
                        </td>
                        <td class="px-2 py-2 text-right text-sm text-gray-300 bg-gray-50">Rp 0</td>
                      </template>
                    </template>
                  </tr>
                </tbody>
                  ...
                  <tr>
                    ...
                    <template v-for="bid in prBids" :key="'a-'+bid.id">
                      <td colspan="2" class="px-3 py-2 text-center border-l-2"
                        ...
                        <button @click="selectBidWinner(bid)"
                          class="px-3 py-1 text-xs rounded"
                          :class="bid.status === 'selected' ? 'bg-green-200 text-green-800 hover:bg-green-300' : 'bg-green-600 text-white hover:bg-green-700'">
                          {{ bid.status === 'selected' ? '✓ Winner' : '✓ Select Winner' }}
                        </button>
                        <div class="text-[10px] text-gray-400 mt-0.5">{{ getVendorWinCount(bid.id) }}/{{ bidItemRows.length }} items</div>
                      </td>
                    </template>
                  </tr>
                </tfoot>
                  ...
                  <div class="text-xs text-gray-600 bg-white p-3 rounded border">
                    ...
                    <div v-if="bidSummary.item_analysis && bidSummary.item_analysis.length > 0" class="mt-2">
                      ...
                      <div class="mt-2 text-gray-500 italic text-[11px]">
                        *Pilih pemenang per item menggunakan radio button (⏺) di tabel harga. Bisa pecah PO ke beberapa vendor.
                      </div>
                    </div>
                  </div>
                </div>
      ...
      <div class="space-y-3">
        ...
        <div class="border border-gray-200 rounded-lg">
          <div class="px-3 py-2 bg-gray-50 border-b flex items-center justify-between">
            <label class="text-sm font-medium text-gray-700">Items dari PR</label>
            <label class="flex items-center gap-1.5 text-xs text-gray-600 cursor-pointer">
              <input type="checkbox" :checked="selectedBidItems.length === formItems.length" @change="toggleSelectAllBidItems" class="w-3.5 h-3.5 rounded border-gray-300 text-blue-600" />
              Select All
            </label>
          </div>
          <div class="max-h-48 overflow-y-auto divide-y divide-gray-100">
            <label v-for="(item, idx) in formItems" :key="idx" class="flex items-center gap-3 px-3 py-2 hover:bg-gray-50 cursor-pointer">
              <input type="checkbox" :value="idx" v-model="selectedBidItems" class="w-4 h-4 rounded border-gray-300 text-blue-600 shrink-0" />
              <div class="flex-1 min-w-0">
                <div class="text-sm font-medium text-gray-800 truncate">{{ item.name || item.productName || '-' }}</div>
                <div class="text-xs text-gray-500">{{ item.qty || 0 }} {{ item.uom || '' }} <span v-if="item.specification" class="text-gray-400">• {{ item.specification }}</span></div>
              </div>
              <div class="text-xs text-gray-400 shrink-0">{{ formatCurrency((item.qty || 0) * (item.price || 0)) }}</div>
            </label>
          </div>
          <div v-if="formItems.length === 0" class="p-3 text-center text-xs text-gray-400">Tidak ada item di PR ini.</div>
        </div>
      </div>
      <div class="mt-5 flex items-center justify-between">
        <span class="text-xs text-gray-500">{{ selectedBidItems.length }}/{{ formItems.length }} items selected</span>
        <div class="space-x-2">
          ...
          <button @click="createBid" :disabled="savingBids || selectedBidItems.length === 0" class="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 text-sm disabled:opacity-50">
...
type PRItem = {
  ...
  specApproval?: number;
  specApprover1?: string | null;
  specApprover2?: string | null;
  specApprovalDate1?: string | null;
  specApprovalDate2?: string | null;
};
...
const specApproval = useApprovalWorkflow('procurement.spec-approval');
const router = useRouter();

// ── Spec Approval helpers ──
const canApproveSpec = (status: number) => specApproval.canApprove(status);
const canRejectSpec = (status: number) => specApproval.canReject(status);

const specApprovalLabel = (status: number) => {
  if (status === 2) return '✓ Approved 2/2';
  if (status === 1) return '⏳ 1/2';
  return '⏳ Pending 0/2';
};
const specApprovalBadgeClass = (status: number) => {
  if (status === 2) return 'bg-green-100 text-green-800';
  if (status === 1) return 'bg-blue-100 text-blue-800';
  return 'bg-yellow-100 text-yellow-800';
};

const approveItemSpec = async (idx: number) => {
  if (!currentPR.value?.id) return;
  const item = formItems.value[idx];
  const currentLevel = item.specApproval || 0;
  const userName = authStore.user?.name || 'Unknown';
  const nextLevel = currentLevel + 1;

  showConfirm(
    `Approve Spec (Level ${nextLevel})`,
    `Approve spec document "${item.attachmentName || 'attachment'}" untuk item "${item.name || item.productName}"?\n\nLevel ${currentLevel}/2 → ${nextLevel}/2`,
    async () => {
      item.specApproval = nextLevel;
      if (nextLevel === 1) {
        item.specApprover1 = userName;
        item.specApprovalDate1 = new Date().toISOString();
      } else if (nextLevel === 2) {
        item.specApprover2 = userName;
        item.specApprovalDate2 = new Date().toISOString();
      }
      // Auto-save the PR notes
      await autoSavePRItems();
    },
    'info',
    'Approve'
  );
...
const rejectItemSpec = async (idx: number) => {
  if (!currentPR.value?.id) return;
  const item = formItems.value[idx];

  showConfirm(
    'Reject Spec',
    `Reject spec approval untuk item "${item.name || item.productName}"?\nStatus akan di-reset ke 0/2.`,
    async () => {
      item.specApproval = 0;
      item.specApprover1 = null;
      item.specApprover2 = null;
      item.specApprovalDate1 = null;
      item.specApprovalDate2 = null;
      await autoSavePRItems();
    },
    'danger',
    'Reject'
...
// Auto-save PR items JSON (for spec approval updates)
const autoSavePRItems = async () => {
  if (!currentPR.value?.id) return;
  try {
    await api.put(`/procurement/purchase-requests/${currentPR.value.id}`, {
      ...
      notes: JSON.stringify({
        ...
        items: formItems.value.map(item => ({
          ...
          specApproval: item.specApproval || 0,
          specApprover1: item.specApprover1 || null,
          specApprover2: item.specApprover2 || null,
          specApprovalDate1: item.specApprovalDate1 || null,
          specApprovalDate2: item.specApprovalDate2 || null,
        })),
        estimatedTotal: estimatedTotal.value
      })
    });
  } catch (err: any) {
    alert(err?.response?.data?.error || 'Failed to save spec approval');
  }
};

// Check if ALL items with attachments are fully spec-approved
const allSpecsApproved = (pr: any): boolean => {
  try {
    const parsed = JSON.parse(pr.notes || '{}');
    const items = parsed.items || [];
    const itemsWithAttachments = items.filter((i: any) => !!i.attachment);
    if (itemsWithAttachments.length === 0) return true; // no attachments = OK
    return itemsWithAttachments.every((i: any) => (i.specApproval || 0) >= 2);
  } catch { return true; }
};
...
const selectedBidItems = ref<number[]>([]);
const toggleSelectAllBidItems = () => {
  if (selectedBidItems.value.length === formItems.value.length) {
    selectedBidItems.value = [];
  } else {
    selectedBidItems.value = formItems.value.map((_: any, i: number) => i);
  }
};
...
const bidItemRows = computed(() => {
  ...
  // Merge all unique items from all bids by item_index
  const itemMap = new Map<number, any>();
  for (const bid of prBids.value) {
    for (const item of (bid.items || [])) {
      if (!itemMap.has(item.item_index)) {
        itemMap.set(item.item_index, { ...item });
      }
    }
  }
  // Sort by item_index and return
  return Array.from(itemMap.values()).sort((a: any, b: any) => a.item_index - b.item_index);
});

const getBidItem = (bid: any, rowIdx: number) => {
  // rowIdx here is the position in bidItemRows, we need to find by item_index
  const targetIndex = bidItemRows.value[rowIdx]?.item_index;
  if (targetIndex === undefined || !bid.items) return { unit_price: 0, total_price: 0, item_index: targetIndex };
  const found = bid.items.find((i: any) => i.item_index === targetIndex);
  return found || { unit_price: 0, total_price: 0, item_index: targetIndex, quantity: 0 };
};

const hasBidItem = (bid: any, rowIdx: number): boolean => {
  const targetIndex = bidItemRows.value[rowIdx]?.item_index;
  if (targetIndex === undefined || !bid.items) return false;
  return bid.items.some((i: any) => i.item_index === targetIndex);
};
...
const createBid = async () => {
  ...
  try {
    ...
    await api.post(`/procurement/purchase-requests/${currentPR.value.id}/bids`, {
      ...
      selected_items: selectedBidItems.value,
    });
...
// Per-item winner selection
const isItemWinner = (bidId: number, rowIdx: number): boolean => {
  const actualItemIndex = bidItemRows.value[rowIdx]?.item_index;
  if (actualItemIndex === undefined) return false;
  const bid = prBids.value.find((b: any) => b.id === bidId);
  if (!bid?.items) return false;
  const item = bid.items.find((i: any) => i.item_index === actualItemIndex);
  return item?.is_winner === 1;
};

const getVendorWinCount = (bidId: number): number => {
  const bid = prBids.value.find((b: any) => b.id === bidId);
  if (!bid?.items) return 0;
  return bid.items.filter((i: any) => i.is_winner === 1).length;
};

const selectItemWinner = async (bid: any, rowIdx: number) => {
  if (!currentPR.value?.id) return;
  const actualItemIndex = bidItemRows.value[rowIdx]?.item_index;
  if (actualItemIndex === undefined) return;
  try {
    // Auto-save prices first if dirty
    if (bidsDirty.value) await saveBids();
    
    await api.post(`/procurement/purchase-requests/${currentPR.value.id}/bids/${bid.id}/select-item/${actualItemIndex}`);
    await loadBids(currentPR.value.id);
    try { await store.fetchPurchaseRequests(); await loadBidProgress(); } catch { /* ignore */ }
  } catch (err: any) {
    alert(err?.response?.data?.error || 'Failed to select item winner');
  }
};
...
const formItems = ref<PRItem[]>([{ productId: null, productName: '', name: '', qty: null, uom: '', specification: '', price: null, attachment: null, attachmentName: null, specApproval: 0, specApprover1: null, specApprover2: null, specApprovalDate1: null, specApprovalDate2: null }]);
...
const resetForm = () => {
  ...
  formItems.value = [{ productId: null, productName: '', name: '', qty: null, uom: '', specification: '', price: null, attachment: null, attachmentName: null, specApproval: 0, specApprover1: null, specApprover2: null, specApprovalDate1: null, specApprovalDate2: null }];
...
const addItemRow = () => {
  formItems.value.push({ productId: null, productName: '', name: '', qty: null, uom: '', specification: '', price: null, attachment: null, attachmentName: null, specApproval: 0, specApprover1: null, specApprover2: null, specApprovalDate1: null, specApprovalDate2: null });
};
...
const uploadItemAttachment = async (idx: number, event: Event) => {
  const input = event.target as HTMLInputElement;
  if (!input.files?.length) return;
  const file = input.files[0];
  const item = formItems.value[idx];
  
  // For new PRs that haven't been saved yet, we need a PR ID.
  // If no currentPR, save the file temporarily and it will be uploaded after save.
  const prId = currentPR.value?.id || 0;
  if (!prId) {
    alert('Please save the PR as Draft first before uploading attachments.');
    input.value = '';
    return;
  }
  ...
  formData.append('file', file);
  try {
    const res = await api.post(`/procurement/purchase-requests/${prId}/item-attachment`, formData, {
      headers: { 'Content-Type': 'multipart/form-data' }
    });
    item.attachment = res.data.file_path;
    item.attachmentName = res.data.original_name || file.name;
  } catch (err: any) {
...
const removeItemAttachment = async (idx: number) => {
  const item = formItems.value[idx];
  if (!item.attachment) return;
  const prId = currentPR.value?.id || 0;
  if (prId && item.attachment) {
    try {
      await api.delete(`/procurement/purchase-requests/${prId}/item-attachment`, {
        params: { file_path: item.attachment }
      });
    } catch { /* ignore delete errors, just clear the reference */ }
  }
...
const convertToPO = async (pr: any) => {
  // Gate: check if all spec attachments are fully approved
  if (!allSpecsApproved(pr)) {
    alert('⚠️ Tidak bisa convert ke PO!\n\nAda dokumen spec teknis yang belum di-approve 2/2.\nPastikan semua attachment spec item sudah mendapat approval penuh sebelum membuat PO.\n\nBuka PR ini dan approve semua spec terlebih dahulu.');
    return;
  }
