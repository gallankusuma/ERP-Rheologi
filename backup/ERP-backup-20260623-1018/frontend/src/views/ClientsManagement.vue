  ...
  <div class="min-h-screen bg-[#f4f6fb] font-sans text-slate-600">
    ...
    <div v-if="showAddClientModal" class="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-50 transition-opacity" @click.self="showAddClientModal = false">
      <div class="bg-white rounded-2xl shadow-2xl max-w-2xl w-full mx-4 max-h-[90vh] overflow-y-auto transform transition-all scale-100">
        ...
        <form @submit.prevent="saveClient" autocomplete="off" class="p-8 space-y-6">
          <div class="grid grid-cols-2 gap-6">
            <div class="space-y-2">
              ...
              <select v-model="clientForm.client_type" required class="w-full px-4 py-2 bg-slate-50 border border-slate-200 rounded-lg focus:bg-white focus:ring-2 focus:ring-primary-100 focus:border-primary-500 transition-all outline-none">
                ...
                <option v-for="cat in clientCategories" :key="cat.id" :value="cat.name.toLowerCase()">{{ cat.name }} ({{ cat.code }})</option>
              </select>
              <p class="text-xs text-slate-400">Code will be auto-generated based on category prefix</p>
            </div>
            <div class="space-y-2">
              <label class="text-sm font-semibold text-slate-700">Company Name <span class="text-red-500">*</span></label>
              <input v-model="clientForm.name" type="text" required autocomplete="off" class="w-full px-4 py-2 bg-slate-50 border border-slate-200 rounded-lg focus:bg-white focus:ring-2 focus:ring-primary-100 focus:border-primary-500 transition-all outline-none" placeholder="Official Company Name" />
            </div>
          </div>
...
const fetchCategories = async () => {
  try {
    ...
    clientCategories.value = res.data?.data || [];
  } catch {
...
const fetchContacts = async () => {
  try {
    ...
    if (searchContacts.value) params.search = searchContacts.value;
    
    const response = await api.get('/clients/contacts/all', { params });
    contacts.value = response.data.data || [];
    contactsPagination.value.total = response.data.pagination?.total || 0;
  } catch (error) {
