<template>
  <div class="p-6 h-[calc(100vh-120px)] flex flex-col">
    ...
    <div class="flex-1 overflow-x-auto">
      <div class="flex gap-4 h-full min-w-max pb-4">
        <div v-for="col in columns" :key="col.id"
          class="w-[300px] shrink-0 bg-gray-50 rounded-xl border flex flex-col transition-all duration-200"
          :class="{'ring-2 ring-indigo-400 bg-indigo-50/40 scale-[1.01]': dragOverCol === col.id}"
          ...
          <!-- Cards -->
          <div class="flex-1 overflow-y-auto p-3 space-y-2 min-h-[100px] transition-colors duration-200 custom-scrollbar"
            @dragover.prevent="onDragOver($event, col.id)"
            @dragleave="onDragLeave(col.id)"
            @drop="onDrop($event, col.id)">
            <div v-for="card in getColCards(col.id)" :key="card.id"
              class="kanban-card bg-white rounded-lg border shadow-sm p-3 cursor-grab active:cursor-grabbing hover:shadow-md transition-all duration-150"
              :class="{'opacity-40 scale-95 ring-2 ring-indigo-300': draggingId === card.id}"
              draggable="true"
              @dragstart="onDragStart($event, card)"
              @dragend="onDragEnd">
              <div class="flex items-start justify-between mb-1">
                <router-link :to="`/rnd/projects/${card.id}`" class="font-medium text-sm text-gray-800 hover:text-indigo-600 leading-tight" @click.stop>{{ card.name }}</router-link>
            ...
            <div v-if="!getColCards(col.id).length" class="text-center text-xs text-gray-400 py-8 border-2 border-dashed border-gray-200 rounded-lg"
              :class="{'border-indigo-300 bg-indigo-50 text-indigo-500': dragOverCol === col.id}">
              {{ dragOverCol === col.id ? '⬇️ Drop here' : 'Drop projects here' }}
            </div>
          </div>
        </div>
    ...
    <Transition name="toast">
      <div v-if="toast.show" class="fixed bottom-6 right-6 z-50 px-4 py-3 rounded-xl shadow-2xl text-sm font-medium flex items-center gap-2"
...
function onDragLeave(colId: string) {
  // Only clear if we're leaving the same column
...
async function onDrop(_e: DragEvent, colId: string) {
  ...
  const oldStatus = dragCard.value.status;
  if (oldStatus === col.status) {
    draggingId.value = null;
    dragCard.value = null;
    return;
  }
  ...
  const cardName = dragCard.value.name;
  dragCard.value.status = col.status;
  draggingId.value = null;

  try {
    await api.patch(`/rnd/projects/${dragCard.value.id}/status`, { status: col.status });
    showToast(`"${cardName}" moved to ${col.label}`);
  } catch (err: any) {
    // Revert on failure
    const card = projects.value.find(p => p.id === dragCard.value?.id);
    if (card) card.status = oldStatus;
    showToast(err.response?.data?.error || 'Failed to move card', 'error');
  }
  dragCard.value = null;
}
