      ...
      <div class="px-4 py-6 sm:px-0">
        ...
        <div class="flex justify-between items-center mb-6">
          <div>
            ...
            <p class="text-xs text-gray-500">Manage product BOMs and approval workflow</p>
          </div>
        ...
        <div v-else class="bg-white shadow overflow-hidden sm:rounded-md">
          <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
              <tr v-for="bom in store.boms" :key="bom.id" class="hover:bg-gray-50">
                ...
                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-2">
                  <button @click="openEditModal(bom.id)" class="text-blue-600 hover:text-blue-900 hover:underline">
                    {{ (bom.approval_status || 0) >= 2 ? 'View' : 'Edit' }}
                  </button>
                  <button 
                  ...
                  <button 
                    ...
                    title="Reopen for editing (reset approval)"
                  >Reopen</button>
                  <button 
      ...
      <div class="bg-white h-screen overflow-y-auto flex flex-col">
        ...
        <div class="border-2 border-dashed border-gray-300 rounded-lg p-6 mb-8">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <!-- Product (Select from Master Data) -->
            <div>
            ...
            <!-- BOM Label / Display Name -->
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-2">BOM Label <span class="text-xs text-gray-400">(auto-filled, editable)</span></label>
              <input 
                v-model="bomHeader.product_name" 
                type="text"
                placeholder="e.g. Cylo Plus @200 ltr (Formulation)"
                :disabled="isFullyApproved"
                class="block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 disabled:bg-gray-100 disabled:cursor-not-allowed"
              />
            </div>
        ...
        <div v-if="!isFullyApproved" class="border-2 border-dashed border-gray-300 rounded-lg p-6 mb-8" data-component-form>
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
            ...
            <div>
              ...
              <select v-model="componentForm.component_id" required class="block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">
            ...
            <div>
              ...
              <input v-model.number="componentForm.quantity" type="number" step="0.01" placeholder="0.00" required class="block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500" />
            </div>

            <!-- UoM -->
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-2">UoM <span class="text-red-500">*</span></label>
              <select v-model="componentForm.unit_id" required class="block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">
            ...
            <div>
              ...
              <input v-model.number="componentForm.loss_percent" type="number" step="0.1" min="0" max="100" placeholder="0" class="block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500" />
            </div>

            <!-- Is Sub-BOM -->
            <div class="flex items-end">
              <label class="inline-flex items-center">
                <input v-model="componentForm.is_sub_bom" type="checkbox" class="form-checkbox text-blue-600 h-5 w-5" />
          ...
          <button @click="addComponent" type="button" class="w-full bg-green-600 text-white px-4 py-2 rounded-md hover:bg-green-700 font-medium">
            + Add Component
          </button>
        </div>
              ...
              <tbody class="bg-white divide-y divide-gray-200">
                <tr v-for="(comp, idx) in bomComponents" :key="idx" class="hover:bg-gray-50">
                  ...
                  <td class="px-6 py-4 whitespace-nowrap text-sm font-medium space-x-3">
                    <button v-if="!isFullyApproved" @click="editComponent(idx)" class="text-blue-600 hover:text-blue-900 font-semibold">Edit</button>
                    <button v-if="!isFullyApproved" @click="removeComponent(idx)" class="text-red-600 hover:text-red-900 font-semibold">Delete</button>
                  </td>
                </tr>
              </tbody>
            </table>
        ...
        <div class="px-6 py-4 border-t border-gray-200 flex justify-between items-center bg-white mt-auto">
          <div class="flex space-x-2">
            ...
            <button 
              ...
              class="px-4 py-2 bg-amber-500 text-white rounded-md hover:bg-amber-600 font-medium"
            >
              ↩ Reopen for Edit
            </button>
            <button 
...
// Only show finished good products (not raw materials)
...
const reopenBOM = async (bomId: number | string) => {
  if (!confirm('Reopen this BOM for editing? This will reset the approval status to Draft.')) return;
  try {
    await api.post(`/bom/${bomId}/reject`);
    alert('BOM reopened for editing');
    await store.fetchBOMs();
    if (showModal.value && editingBomId.value) {
      currentBOMApprovalStatus.value = 0;
    }
  } catch (error: any) {
    alert(error.response?.data?.error || 'Failed to reopen BOM');
  }
};
...
const saveBOM = async () => {
  try {
    if (!bomHeader.value.product_id) {
      alert('Please select a product from master data');
      return;
    }
    if (!bomHeader.value.product_name) {
      // Auto-fill from selected product
      const product = productStore.products.find(p => p.id == bomHeader.value.product_id);
      bomHeader.value.product_name = product?.name || '';
    }
    ...
    const bomData = {
      product_id: Number(bomHeader.value.product_id),
      product_name: bomHeader.value.product_name,
      notes: bomHeader.value.notes,
      details: details
    };

    if (isEditing.value && editingBomId.value) {
      await api.put(`/bom/${editingBomId.value}`, bomData);
    } else {
      await api.post('/bom', bomData);
    }
...
const deleteBOM = async (bomId: string | number) => {
  if (confirm('Delete this BOM? This action cannot be undone.')) {
    try {
